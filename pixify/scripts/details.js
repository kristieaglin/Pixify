...
window.onload = function() {
    ...
    const currentSub = imageSets.filter(item => item.id == currentId);
    console.log(currentSub);

    const sub = `<div class="current-sub">
    <h2 class="current-title">${currentSub[0].title}</h2>
    <p class="current-name">${currentSub[0].name}</p>
    <img src="${currentSub[0].images[0]}" alt="${currentSub[0].title}" class="current-img" />
    <p class="current-number"><span>1</span> of 5</p>
    <div class="btn-div">
    <button class="sub-btn prev-btn">Previous</button>
    <button class="sub-btn next-btn">Next</button>
    </div>
    </div>`;
...
// NEXT & PREVIOUS BUTTONS
    ...
    const nextButton = () => {
        let imageNumber = document.querySelector("span");
        const currentImg = document.querySelector(".current-img");
        console.log(currentImg);
        for (let i = 0; i < currentSub[0].images.length; i++) {
            console.log(currentImg.setAttribute("src", currentSub[0].images[i]))
            console.log(i);
            console.log(currentSub[0].images[i])
            currentImg.setAttribute("src", currentSub[0].images[i]);
            imageNumber.innerHTML = i + 1;
        };
    ...
    const prevButton = () => {
        ...
        const currentImg = document.querySelector(".current-img");
        console.log(currentImg);
        for (let i = currentSub[0].images.length; i >= 0; i--) {
            console.log(currentImg.setAttribute("src", currentSub[0].images[i]))
            console.log(i);
            console.log(currentSub[0].images[i])
            currentImg.setAttribute("src", currentSub[0].images[i]);
...
const toggleMode = () => {
   ...
   const btn1 = document.querySelector(".prev-btn");
   const btn2 = document.querySelector(".next-btn");
   ...
   if(body.style.backgroundColor === "var(--grayBlue)"){
    ...
    btn1.style.border = "1px solid var(--neonBlue)";
    btn2.style.border = "1px solid var(--neonBlue)";
   }
